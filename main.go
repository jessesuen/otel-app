...
func readFileOrEnv(name string) string {
...
func init() {
	var err error
	color = readFileOrEnv("COLOR")

	envLatencyStr := readFileOrEnv("LATENCY")
	if envLatencyStr != "" {
	...
	envErrorRateStr := readFileOrEnv("ERROR_RATE")
	if envErrorRateStr != "" {
...
func main() {
	...
	if tls {
		tlsConfig, err := CreateServerTLSConfig("", "", []string{"localhost", "otel-app"})
		if err != nil {
